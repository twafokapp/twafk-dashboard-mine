...
import type { OrderedProducts } from './orderedProducts.type.ts';
import { Avatar, AvatarImage } from '@/components/ui/avatar';
import { AppButton } from '@/common/appButton/appButton.tsx';
import { useNavigate } from 'react-router';

const columnsHelper = createColumnHelper<OrderedProducts>();
export const UseClientColumns = () => {
  ...
  return [
    ...
    columnsHelper.accessor('phone_number', {
      ...
      header: () => (
        <span className='flex justify-center '>{t('requested_products.phone_number')}</span>
      ),
    }),
    columnsHelper.accessor('city', {
      cell: info => info.getValue(),

      header: () => <span className='flex justify-center '>{t('requested_products.city')}</span>,
    }),
    columnsHelper.accessor('number_of_products', {
      cell: info => info.getValue(),

      header: () => (
        <span className='flex justify-center '>{t('most_wanted_clients.number_of_products')}</span>
      ),
    }),
