...
import type { MostWantedClients } from './mostWantedClients.type.ts';
import { Avatar, AvatarImage } from '@/components/ui/avatar';
import { AppButton } from '@/common/appButton/appButton.tsx';
import { useNavigate } from 'react-router';

const columnsHelper = createColumnHelper<MostWantedClients>();
export const UseClientColumns = () => {
  ...
  return [
    ...
    columnsHelper.accessor('number_of_orders', {
      cell: info => info.getValue(),
      header: () => (
        <span className='flex justify-center '>{t('most_wanted_clients.number_of_requests')}</span>
      ),
    }),
